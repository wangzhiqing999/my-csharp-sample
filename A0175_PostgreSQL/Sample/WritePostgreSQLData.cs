...
WHERE 
  ...
  AND sale_item = :sale_item";
        ...
        /// 用于 测试数据的 日期
        /// </summary>
        private static DateTime TEST_SALE_DATE = new DateTime(2014, 12, 21);
        ...
        /// <returns></returns>
        private bool ExistData(NpgsqlConnection conn)
        {
            // 创建一个 Command.
            NpgsqlCommand existCommand = conn.CreateCommand();

            // 定义需要执行的SQL语句.
            existCommand.CommandText = EXIST_SQL;

            // 定义要查询的参数.
            existCommand.Parameters.Add(new NpgsqlParameter (":sale_date", TEST_SALE_DATE));
            existCommand.Parameters.Add(new NpgsqlParameter(":sale_item", TEST_SALE_ITEM));
        ...
        /// </summary>
        /// <param name="conn"></param>
        private void InsertData(NpgsqlConnection conn)
        {
            // 创建一个 Command.
            NpgsqlCommand insertCommand = conn.CreateCommand();

            // 定义需要执行的SQL语句.
            insertCommand.CommandText = INSERT_SQL;

            // 定义要查询的参数.
            insertCommand.Parameters.Add(new NpgsqlParameter(":sale_date", TEST_SALE_DATE));
            insertCommand.Parameters.Add(new NpgsqlParameter(":sale_item", TEST_SALE_ITEM));
            insertCommand.Parameters.Add(new NpgsqlParameter(":sale_money", 100000));
        ...
        /// </summary>
        /// <param name="conn"></param>
        private void ShowData(NpgsqlConnection conn)
        {
            // 创建一个 Command.
            NpgsqlCommand selectCommand = conn.CreateCommand();

            // 定义需要执行的SQL语句.
            selectCommand.CommandText = SELECT_SQL;

            // 定义要查询的参数.
            selectCommand.Parameters.Add(new NpgsqlParameter(":sale_date", TEST_SALE_DATE));
            selectCommand.Parameters.Add(new NpgsqlParameter(":sale_item", TEST_SALE_ITEM));

            // 执行SQL命令，结果存储到Reader中.
            NpgsqlDataReader testReader = selectCommand.ExecuteReader();
        ...
        /// </summary>
        /// <param name="conn"></param>
        private void UpdateData(NpgsqlConnection conn)
        {
            // 创建一个 Command.
            NpgsqlCommand updateCommand = conn.CreateCommand();

            // 定义需要执行的SQL语句.
            updateCommand.CommandText = UPDATE_SQL;

            // 定义要查询的参数.
            updateCommand.Parameters.Add(new NpgsqlParameter(":sale_date", TEST_SALE_DATE));
            updateCommand.Parameters.Add(new NpgsqlParameter(":sale_item", TEST_SALE_ITEM));
            updateCommand.Parameters.Add(new NpgsqlParameter(":sale_money", 0.1));
        ...
        private void DeleteData(NpgsqlConnection conn)
        {
            // 创建一个 Command.
            NpgsqlCommand deleteCommand = conn.CreateCommand();

            // 定义需要执行的SQL语句.
            deleteCommand.CommandText = DELETE_SQL;

            // 定义要查询的参数.
            deleteCommand.Parameters.Add(new NpgsqlParameter(":sale_date", TEST_SALE_DATE));
            deleteCommand.Parameters.Add(new NpgsqlParameter(":sale_item", TEST_SALE_ITEM));
        ...
        /// </summary>
        /// <param name="conn"></param>
        private void Transaction(NpgsqlConnection conn)
        {
        ...
        /// </summary>
        /// <param name="conn"></param>
        private void InsertData(NpgsqlConnection conn, NpgsqlTransaction t)
        {
            // 创建一个 Command.
            NpgsqlCommand insertCommand = conn.CreateCommand();
