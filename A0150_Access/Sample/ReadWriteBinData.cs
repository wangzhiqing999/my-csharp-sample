...
namespace A0150_Access.Sample
{
    ...
    public class ReadWriteBinData
    {
        ...
        /// 用于查询 数据是否存在的 SQL 语句.
        /// </summary>
        private const String EXIST_SQL = "SELECT COUNT(Test_Name) FROM Test_BinData WHERE [Test_Name] = ?";


        /// <summary>
        /// 插入的 sql 语句.
        /// </summary>
        private const string INSERT_SQL = @"INSERT INTO Test_BinData (Test_Name, Test_Data) VALUES(?,?)";


        /// <summary>
        /// 查询的 sql 语句.
        /// </summary>
        private const string SELECT_SQL = @"SELECT * FROM Test_BinData WHERE Test_Name = ?";


        /// <summary>
        /// 用于删除的 SQL 语句.
        /// </summary>
        private const String DELETE_SQL = "DELETE FROM [Test_BinData] WHERE [Test_Name] = ?";
        ...
        private void InsertData(OleDbConnection conn)
        {
            ...
            insertCommand.Parameters.Add(new OleDbParameter("?", buffer));
